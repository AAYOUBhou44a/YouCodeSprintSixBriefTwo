...
<div class="max-w-3xl mx-auto w-full">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form action="{{isset($sprint) ? '/sprints/'.$sprint->id : '/sprints'}}" method="POST" class="p-8 space-y-8">
            @csrf
            @if(isset($sprint))
                @method('PUT')
            @endif
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" placeholder="ex: Sprint 03 - Approfondissement Frameworks" required name="name" value="{{$sprint->name?? old('name')}}"
            ...
            <div class="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{isset($sprint)? $sprint->start_date->format('Y-m-d') : old('start_date')}}" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-slate-600 transition">
                        @error('start_date')
                            <span style="color: red; font-size: 12px;">{{$message}}</span>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="date" name="end_date" value="{{ isset($sprint) ? $sprint->end_date->format('Y-m-d') : old('end_date')}}" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-slate-600 transition">
                        @error('end_date')
                            <span style="color: red; font-size: 12px;">{{$message}}</span>
                        @enderror
                    </div>
